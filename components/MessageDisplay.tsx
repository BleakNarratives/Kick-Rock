...
const MessageDisplay: React.FC<MessageDisplayProps> = ({ message, error, isLoading }) => {
  ...
  const handleShareEmail = () => {
    window.location.href = `mailto:?subject=${encodeURIComponent("Your Grand Goodbye Message")}&body=${encodeURIComponent(message)}`;
  };
  ...
  const handleShareNative = () => {
    if (navigator.share) {
      navigator.share({
        title: 'The Grand Goodbye Message',
        text: message,
      })
      .then(() => console.log('Shared successfully'))
      .catch((error) => console.error('Error sharing:', error));
    } else {
      alert("Your browser does not support the Web Share API. Please use copy or other specific share options.");
    }
  };
  ...
  if (isLoading && !message && !error) {
    return (
      <div className="w-full max-w-md bg-white p-6 rounded-xl shadow-2xl mt-8 text-center text-gray-600">
        <p className="text-lg">Hang tight, crafting your perfect farewell...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-md bg-white p-6 rounded-xl shadow-2xl mt-8">
      ...
      {message ? (
        <>
          <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">Your Grand Goodbye:</h2>
          <div className="whitespace-pre-wrap font-mono text-gray-700 bg-gray-50 p-4 rounded-lg border border-gray-200 overflow-auto max-h-96">
            {message}
          </div>
          <div className="mt-4 flex flex-col sm:flex-row gap-2">
            ...
            <button
              ...
              className={`flex-1 py-2 px-4 rounded-lg font-semibold transition duration-300
                ${isSpeaking ? 'bg-pink-500 text-white animate-pulse' : 'bg-purple-500 text-white hover:bg-purple-600'}
                ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={isLoading}
            >
              {isSpeaking ? 'Stop Listening' : 'Listen to Message'}
            </button>
          </div>

          <div className="mt-6 pt-4 border-t border-gray-200">
            <h3 className="text-lg font-bold text-gray-800 mb-3 text-center">Share Your Farewell:</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <button onClick={handleShareSMS} className="flex items-center justify-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 transition duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 mr-1 text-gray-700">
              ...
              <button onClick={handleShareEmail} className="flex items-center justify-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 transition duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 mr-1 text-gray-700">
              ...
              <button onClick={handleShareWhatsApp} className="flex items-center justify-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 transition duration-200">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 mr-1 text-gray-700">
              ...
              {navigator.share && (
                <button onClick={handleShareNative} className="flex items-center justify-center p-3 border border-gray-300 rounded-lg hover:bg-gray-100 transition duration-200">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 mr-1 text-gray-700">
        ...
        <p className="text-gray-600 text-center text-lg italic">
          Your unique breakup message will appear here.
        </p>
      )}
    </div>
  );
